    ...
    if (user_bot_input_text!=""){
        ...
        let chat_input_data={
            ...
            $.ajax({
                ...
                dataType:"json",
                contentType:'application/json',
                success: function (result) { $("#chat_messages").append('<div class="bot__messages">' + result.bot_response + ' </div>') $('.chatbox__messages__cotainer').animate({ scrollTop: $('.chatbox__messages__cotainer')[0].scrollHeight}, 1000)},
                error:function(result){
                    alert(result.responseJSON.message)
                }
            });
